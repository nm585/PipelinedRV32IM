...
ARCHITECTURE struct OF tb_RV32I IS
	...
	-- IPC / breakpoint support (LAB5 page 8)
...
BEGIN
	...
	PORT MAP (
		...
		mclk_cnt_o				=> mclk_cnt_o,				-- TOP output (CLKCNT_o)

		--IPC / breakpoint support (LAB5 page 8)
		BPADDR_i					=> BPADDR_i,					-- breakpoint addr (SW7-SW0)
		STCNT_o					=> STCNT_o,						-- stall counter
		FHCNT_o					=> FHCNT_o,						-- flush counter
		st_trigger_o			=> st_trigger_o				-- Signal-Tap trigger
	);
--------------------------------------------------------------------	
